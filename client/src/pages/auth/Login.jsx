...
const Login = () => {
  ...
  const LoginHandler = async ({ email, password }) => {
    try {
      ...
      toast.error(err ? err : 'Invalid email or password');
    }
  };
    ...
    <div className='min-h-screen w-full flex justify-center items-center bg-gray-50 py-10 px-4'>
      ...
      <div className='w-full max-w-sm bg-white p-6 rounded-xl shadow-lg border border-gray-100'>
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <p className='text-gray-500 text-xs mt-1'>Log in to your BookSwap account</p>
        </div>
        ...
        <form onSubmit={handleSubmit(LoginHandler)} className='flex flex-col gap-4'>
          ...
          <div className="flex flex-col gap-1">
            <div className="flex justify-between items-center">
              ...
              <span onClick={() => toast("Redirect to forgot password")} className="text-xs text-orange-600 hover:underline cursor-pointer">
                Forgot Password?
              </span>
            </div>
          ...
          <button
            type="submit"
            className='mt-2 w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-2.5 rounded-lg transition-colors text-sm shadow-md shadow-orange-500/20'
          >
            Log In
          </button>
        </form>
        ...
        <div className="mt-5 text-center text-xs text-gray-600">
          ...
          <button onClick={() => navigate('/sign-up')} className="text-orange-600 font-bold hover:underline">
            Register here
          </button>
        </div>
